  ...
  delay(300);
    uint32_t c = strip.Color(255, 255, 255);
      ...
      for (int i = 0; i < random(1, 7); i++) {
        randomSeed(random(0, 12345));
    strip.setPixelColor(random(0, 25), c);
  }
...
void delayWithCheck(long time) {
  for (long i = 0; i < time; i += 5) {
    delay(5);
    if (checkButtonPress()) return;
  }
}

void slowFlashRandom(int wait) {
  delay(300);
      for (int i=0; i < strip.numPixels(); i++) {
          strip.setPixelColor(i, 0);        //turn every pixel off
      }
